...
def write_to_file(filename, signal, samplerate=48000):
...
def adjust_to_3_sec(monoSignal, sampleRate):
    num_samples_3s = sampleRate * 3
    if len(monoSignal) <= num_samples_3s:
        times = num_samples_3s // len(monoSignal)
        extended_signal = np.tile(monoSignal, times)
        fill_rest = monoSignal[:num_samples_3s-len(extended_signal)]
        return np.concatenate([np.tile(monoSignal, times), fill_rest])
    if len(monoSignal) > num_samples_3s:
        start = np.random.randint(0,len(monoSignal)-num_samples_3s)
        piece = monoSignal[start:start+num_samples_3s]
        # print(len(piece), num_samples_3s)
        assert len(piece) == num_samples_3s
        return piece
...
def iterate_convolute_save(monoSignal, sofa, path, norm="peak"):
    ...
    for angle in range(90, 271):
        ...
        write_to_file(path + "/" + str(realAngle) + ".wav", convolved, sampleRate)
    #     result.append(np.mean(convolvedLeft)/ np.mean(convolvedRight))
    # return result

def iterate_over_directory(path, sofa, write_path, samplerate):
    """
    iterates over directory with wav files
    adjusts them to length of 3s
    uses iterate_convolute_save to create directory of spatially located files
    :return:
    """
    ...
    for file in directory:
        if file.suffix != ".wav":
            continue
        print(f"creating directory for {file}")
        signal = load_sound_mono(str(file))
        signal = adjust_to_3_sec(signal, samplerate)
        # print(len(signal))
        iterate_convolute_save(signal, sofa, write_path+"/"+file.stem, norm="lufs")
...
iterate_over_directory("C:/Users/uzytek/PycharmProjects/inzynierka/originalSoundDB/3s_aiff_ogg_mp3_flac", sofa, "D:/inzynierka_baza/wynikowy",sampleRate)